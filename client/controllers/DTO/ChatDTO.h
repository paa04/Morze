...
public:
    ...
    ChatDTO(std::string id, std::string roomId, std::string type, std::string title,
            std::string createdAt, std::string lastMessageNumber)
        : id_(std::move(id)), roomId_(std::move(roomId)), type_(std::move(type)),
          title_(std::move(title)), createdAt_(std::move(createdAt)), lastMessageNumber_(std::move(lastMessageNumber)) {
    }
