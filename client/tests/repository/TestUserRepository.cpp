...
TEST_CASE("UserRepository") {
    ...
    UserRepository repo(ioc, storage);
    ...
    SUBCASE("getAllUsers returns multiple users") {
        ...
        run_async(ioc, repo.addUser(UserDAOConverter::convert(u1)));
        run_async(ioc, repo.addUser(UserDAOConverter::convert(u2)));

        auto all = run_async(ioc, repo.getAllUsers());
        CHECK(all.size() == 2);
    ...
    SUBCASE("getUserByLogin returns user when exists") {
        ...
        run_async(ioc, repo.addUser(dao));

        auto retrieved = run_async(ioc, repo.getUserByLogin("alice"));
    ...
    SUBCASE("getUserByLogin throws NotFoundError when user does not exist") {
        CHECK_THROWS_AS(run_async(ioc, repo.getUserByLogin("nobody")),
                        UserNotFoundError);
    }
    ...
    SUBCASE("getUserByMemberId returns user when exists") {
        ...
        auto retrieved = run_async(ioc, repo.getUserByMemberId(member_id));
    ...
    SUBCASE("getUserByMemberId throws NotFoundError when member_id not found") {
        ...
        CHECK_THROWS_AS(run_async(ioc, repo.getUserByMemberId(non_existent)),
                        UserNotFoundError);
    }
    ...
    SUBCASE("addUser throws AlreadyExistsError when login already exists") {
        ...
        auto dupDao = UserDAOConverter::convert(duplicate);
        CHECK_THROWS_AS(run_async(ioc, repo.addUser(dupDao)),
    ...
    SUBCASE("updateUser modifies existing user") {
        ...
        auto updated_dao = UserDAOConverter::convert(user);
        run_async(ioc, repo.updateUser(updated_dao));
    ...
    SUBCASE("updateUser throws NotFoundError when user does not exist") {
        ...
        auto dao = UserDAOConverter::convert(user);
        CHECK_THROWS_AS(run_async(ioc, repo.updateUser(dao)),
                        UserNotFoundError);
    }
    ...
    SUBCASE("removeUser deletes existing user") {
        ...
        auto dao = UserDAOConverter::convert(user);
        run_async(ioc, repo.addUser(dao));
        CHECK(run_async(ioc, repo.getAllUsers()).size() == 1);

        run_async(ioc, repo.removeUser("todelete"));
        CHECK(run_async(ioc, repo.getAllUsers()).empty());
    }

    SUBCASE("removeUser does not throw when user does not exist") {
        CHECK_NOTHROW(run_async(ioc, repo.removeUser("nobody")));
    }
}