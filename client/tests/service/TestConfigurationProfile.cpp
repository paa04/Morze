...
TEST_CASE("ConnectionProfileService") {
    asio::io_context ioc;
    ...
    auto repo = std::make_shared<ConnectionProfileRepository>(ioc, storage);
    ConnectionProfileService service(repo);
    ...
    SUBCASE("addProfile inserts profile successfully") {
        ...
        run_async(ioc, service.addProfile(profile));

        auto all = run_async(ioc, service.getAllProfiles());
        CHECK(all.size() == 1);
    ...
    SUBCASE("addProfile throws AlreadyExistsError when profile with same id exists") {
        ...
        CHECK_THROWS_AS(run_async(ioc, service.addProfile(profile)),
    ...
    SUBCASE("getProfileById returns profile when exists") {
        ...
        auto retrieved = run_async(ioc, service.getProfileById(profile.getId()));
    ...
    SUBCASE("getProfileById throws NotFoundError when profile does not exist") {
        ...
        CHECK_THROWS_AS(run_async(ioc, service.getProfileById(non_existent_id)),
    ...
    SUBCASE("updateProfile modifies existing profile") {
        ...
        run_async(ioc, service.updateProfile(profile));
    ...
    SUBCASE("updateProfile throws NotFoundError when profile does not exist") {
        auto profile = create_test_profile();
        CHECK_THROWS_AS(run_async(ioc, service.updateProfile(profile)),
                        ConnectionProfileNotFoundError);
    }
    ...
    SUBCASE("removeProfile deletes existing profile") {
        auto profile = create_test_profile();
        run_async(ioc, service.addProfile(profile));
        CHECK(run_async(ioc, service.getAllProfiles()).size() == 1);

        run_async(ioc, service.removeProfile(profile.getId()));
        CHECK(run_async(ioc, service.getAllProfiles()).empty());
    }

    SUBCASE("removeProfile does not throw when profile does not exist") {
        auto non_existent_id = uuid_gen();
        CHECK_NOTHROW(run_async(ioc, service.removeProfile(non_existent_id)));
    }
    ...
    SUBCASE("getAllProfiles returns profiles sorted by updated_at desc") {
        ...
        run_async(ioc, service.addProfile(p1));
        run_async(ioc, service.addProfile(p2));
        run_async(ioc, service.addProfile(p3));

        auto result = run_async(ioc, service.getAllProfiles(true));
        REQUIRE(result.size() == 3);

        CHECK(result[0].getId() == p2.getId());
    ...
    SUBCASE("getLatestProfile returns most recently updated profile") {
        ...
        auto latest = run_async(ioc, service.getLatestProfile());
        CHECK(latest.getId() == p2.getId());
    }

    SUBCASE("getLatestProfile throws NotFoundError when no profiles exist") {
        CHECK_THROWS_AS(run_async(ioc, service.getLatestProfile()),
                        ConnectionProfileNotFoundError);
    }
}