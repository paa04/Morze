...
TEST_CASE("MessageService") {
    ...
    auto repo = std::make_shared<MessageRepository>(ioc, storage);
    MessageService service(repo);
    ...
    SUBCASE("getMessageById returns message when exists") {
        ...
        run_async(ioc, service.addMessage(msg));

        auto retrieved = run_async(ioc, service.getMessageById(msg.getMessageId()));
    ...
    SUBCASE("getMessageById throws NotFoundError when message does not exist") {
        ...
        CHECK_THROWS_AS(run_async(ioc, service.getMessageById(non_existent_id)),
    ...
    SUBCASE("getAllMessages returns all messages") {
        ...
        run_async(ioc, service.addMessage(m1));
        run_async(ioc, service.addMessage(m2));

        auto all = run_async(ioc, service.getAllMessages());
        CHECK(all.size() == 2);
    }
    ...
    SUBCASE("updateMessage modifies existing message") {
        ...
        run_async(ioc, service.updateMessage(msg));
    ...
    SUBCASE("updateMessage throws NotFoundError when message does not exist") {
        auto msg = create_test_message("Hello", chat1_id, sender1_id, std::chrono::system_clock::now());
        CHECK_THROWS_AS(run_async(ioc, service.updateMessage(msg)),
                        MessageNotFoundError);
    }
    ...
    SUBCASE("addMessage throws AlreadyExistsError when message with same id exists") {
        ...
        CHECK_THROWS_AS(run_async(ioc, service.addMessage(msg)),
    ...
    SUBCASE("removeMessage deletes existing message") {
        auto msg = create_test_message("Hello", chat1_id, sender1_id, std::chrono::system_clock::now());
        run_async(ioc, service.addMessage(msg));
        CHECK(run_async(ioc, service.getAllMessages()).size() == 1);

        run_async(ioc, service.removeMessage(msg.getMessageId()));
        CHECK(run_async(ioc, service.getAllMessages()).empty());
    }

    SUBCASE("removeMessage does not throw when message does not exist") {
        auto non_existent_id = uuid_gen();
        CHECK_NOTHROW(run_async(ioc, service.removeMessage(non_existent_id)));
    }
    ...
    SUBCASE("getMessagesByChatId returns messages sorted by created_at desc") {
        ...
        run_async(ioc, service.addMessage(m1));
        run_async(ioc, service.addMessage(m2));
        run_async(ioc, service.addMessage(m3));

        auto result = run_async(ioc, service.getMessagesByChatId(chat1_id));
    ...
    SUBCASE("getMessagesByChatId returns empty list for chat with no messages") {
        ...
        auto result = run_async(ioc, service.getMessagesByChatId(empty_chat_id));
        CHECK(result.empty());
    }
    ...
    SUBCASE("getMessagesByChatAndSender returns messages from sender in specific chat sorted") {
        ...
        run_async(ioc, service.addMessage(m1));
        run_async(ioc, service.addMessage(m2));
        run_async(ioc, service.addMessage(m_other));

        auto result = run_async(ioc, service.getMessagesByChatAndSender(chat1_id, sender1_id));
    ...
    SUBCASE("getMessagesByChatAndSender returns empty list when sender has no messages in chat") {
        ...
        auto result = run_async(ioc, service.getMessagesByChatAndSender(chat1_id, new_sender_id));
        CHECK(result.empty());
    }
    ...
    SUBCASE("getMessagesByChatFromDate returns messages from given date onwards sorted") {
        ...
        auto result = run_async(ioc, service.getMessagesByChatFromDate(chat1_id, base));
        REQUIRE(result.size() == 2);
    ...
    SUBCASE("getMessagesByChatFromDate returns empty list when no messages after date") {
        ...
        auto result = run_async(ioc, service.getMessagesByChatFromDate(chat1_id, future));
        CHECK(result.empty());
    }
    ...
    SUBCASE("getMessagesBySender returns all messages from sender across all chats sorted") {
        ...
        auto result = run_async(ioc, service.getMessagesBySender(sender1_id));
        REQUIRE(result.size() == 2);
        CHECK(result[0].getMessageId() == m2.getMessageId());
        CHECK(result[1].getMessageId() == m1.getMessageId());
    }
    ...
    SUBCASE("getMessagesBySender returns empty list when sender has no messages") {
        ...
        auto result = run_async(ioc, service.getMessagesBySender(new_sender));
        CHECK(result.empty());
    }
    ...
    SUBCASE("getMessagesByChatSenderFromDate returns filtered and sorted messages") {
        ...
        run_async(ioc, service.addMessage(m2));
        run_async(ioc, service.addMessage(m3));
        run_async(ioc, service.addMessage(m_other_sender));
        run_async(ioc, service.addMessage(m_other_chat));

        auto result = run_async(ioc, service.getMessagesByChatSenderFromDate(chat1_id, sender1_id, base));
        REQUIRE(result.size() == 2);
        CHECK(result[0].getMessageId() == m3.getMessageId());
        CHECK(result[1].getMessageId() == m2.getMessageId());
    }
    ...
    SUBCASE("getMessagesByChatSenderFromDate returns empty list when no matching messages") {
        auto future = std::chrono::system_clock::now() + std::chrono::hours(24);
        auto result = run_async(ioc, service.getMessagesByChatSenderFromDate(chat1_id, sender1_id, future));
        CHECK(result.empty());
    }
}