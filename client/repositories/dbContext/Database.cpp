...
bool Database::runMigrations() {
    ...
    // Начинаем транзакцию через ORM (она будет применена к тому же файлу БД)
